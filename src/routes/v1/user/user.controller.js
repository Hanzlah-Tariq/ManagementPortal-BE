...
export const registerUser = asyncHandler(async (ctx) => {
  let user;
  const { user_name, email, password, type_of_id, secret } = ctx.request.body;
  ...
  user = await UserModel.findOne({ user_name });
  if (user) {
  ...
  await pollModel.create({ dbStatus: 'inactive', user: createdUser.id })
  ctx.status = 201;
