...
export const addEmp = asyncHandler(async (ctx) => {
  ...
  let typeOfId = 0
  const { name, email, designation } = ctx.request.body;
  ...
  if (designation === "doctor") {
    typeOfId = 2
  } else if (designation === "compounder") {
    typeOfId = 3
  } else {
    typeOfId = 0
  }
  const empAccount = await UserModel.create({ username: name, email, password, typeOfId });
  const poll = await pollModel.create({ dbStatus: 'inactive', user: empAccount.id, typeOfId: typeOfId })
  // add employee to request body so that it relates to its specific account
  ctx.request.body.user = empAccount.id;
  ctx.request.body.poll = poll.id;
  // save employee data
...
export const uptEmp = asyncHandler(async (ctx) => {
  ...
  const password = await bcryptJs.hash(_password, salt);
  await UserModel.findByIdAndUpdate(empData.user, { username: name, email, password, typeOfId }, { new: true, runValidators: true });
  await pollModel.findByIdAndUpdate(empData.poll, { typeOfId },{ new: true, runValidators: true });
  ctx.status = 200;
