...
import bcryptJs from "bcryptjs"; // @desc      create employee
...
export const addEmp = asyncHandler(async ctx => {
  ...
  const password = 'test123'; // creates employee account
  ...
  ctx.request.body.poll = poll.id; // save employee data
...
export const uptEmp = asyncHandler(async ctx => {
  ...
  await pollModel.findByIdAndUpdate(empData.poll, {
    typeOfId
  }, {
    new: true,
    runValidators: true
