...
import { DATE } from "../../../common/constants"; // @desc      create patient
...
export const addPatient = asyncHandler(async ctx => {
  ...
  const {
    ...
    medicine,
    disease
  } = ctx.request.body; // add patient to request body so that it relates to its specific account
...
export const uptPatient = asyncHandler(async ctx => {
  ...
  const patient = await patientModel.findById(ctx.params.id);
  ctx.assert(patient, 404, 'Patient not found with this id'); // update patient related data
  ...
  await patientModel.findByIdAndUpdate(ctx.params.id, ctx.request.body, {
    new: true,
    runValidators: true
  });
  ctx.status = 200;
  ctx.body = {
    success: true,
    status: 'Patient Successfully Updated'
  };
...
export const patientRecord = asyncHandler(async ctx => {
  const record = await patientModel.findOne({
    user: ctx.user.id
  });
