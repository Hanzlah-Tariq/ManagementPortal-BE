...
import asyncHandler from '../../../middleware/async'; // @desc      get all medicines
...
export const addMedicine = asyncHandler(async ctx => {
  ctx.assert(ctx.request.body, 400, 'Please enter the  required fields'); // save medicine data

  await medicineModel.create(ctx.request.body);
  ctx.status = 201;
  ctx.body = {
    success: true,
    status: 'Medicine Successfully Added'
  };
...
export const updateMedicine = asyncHandler(async ctx => {
  ...
  await medicineModel.findByIdAndUpdate(ctx.params.id, ctx.request.body, {
    new: true,
    runValidators: true
  });
  ctx.status = 200;
  ctx.body = {
    success: true,
    status: 'Medicine Successfully Updated'
  };
...
export const deleteMedicine = asyncHandler(async ctx => {
  ...
  ctx.body = {
    success: true,
    status: 'Medicine Successfully Deleted'
  };
});